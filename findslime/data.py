...
# ========== 获取所有不同count的数量 ==========
def get_distinct_counts(chunk_size=1000000):
    """
    获取数据库中所有不同的count值及其出现次数（分块处理）
    
    参数:
        chunk_size (int): 每次读取的记录数，默认10000
    ...
    try:
        ...
        for chunk in pd.read_sql_query(query, conn, chunksize=chunk_size):
            # 统计当前块中每个count值的出现次数
            chunk_counts = chunk['count'].value_counts()
            
            # 累加到总统计中
            for count_val, freq in chunk_counts.items():
                if count_val in count_frequency:
                    count_frequency[count_val] += freq
                else:
                    count_frequency[count_val] = freq
        ...
        df_result = pd.DataFrame([
            {'count': k, 'frequency': v} 
            for k, v in sorted(count_frequency.items(), key=lambda x: x[0], reverse=True)
        ])
...
# ========== 新增：优化版本的获取前N个count值的方法 ==========
def get_top_n_counts(n=3, chunk_size=10000):
    """
    获取前N个count值（优化版本，适用于大数据集）
